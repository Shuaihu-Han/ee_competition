...
def extract_all_items_without_oracle(model_trigger, model_arg, device, idx, content: str, token, seg, mask, seq_len, threshold_0, threshold_1, threshold_2, threshold_3, threshold_4, id_type: dict, id_args: dict, ty_args_id: dict):
    assert token.size(0) == 1
    content = content[0]
    result = {'id': idx, 'content': content}
    # text_emb = model.plm(token, seg, mask)
    text_emb_trigger = model_trigger.plm(token, seg, mask)
    text_emb_arg = model_arg.plm(token, seg, mask)

    # args_id = {id_args[k]: k for k in id_args}
    # args_len_dict = {args_id[k]: ARG_LEN_DICT[k] for k in ARG_LEN_DICT}

    p_type, type_emb = model_trigger.predict_type(text_emb_trigger, mask)

    p_type_arg, type_emb_arg = model_arg.predict_type(text_emb_arg, mask)
    ...
    for type_pred_one in type_pred:
        ...
        p_s, p_e, text_rep_type = model_trigger.predict_trigger(type_rep, text_emb_trigger, mask)
        ...
        for k, span in enumerate(trigger_spans):
            ...
            p_s, p_e, type_soft_constrain = model_arg.predict_args(text_emb_arg, rp, tm, mask, type_rep_arg)

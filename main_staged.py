...
from models.staged_model import CasEE as CasEE_TRIGGER
...
def main():
    ...
    framework = Framework(config, model_trigger, model_arg)
    ...
    if config.generate_result:
        ...
        prf_s, pred_records = framework.evaluate_without_oracle(config, model_trigger, model_arg, dev_loader, config.device, config.seq_length, config.id_type, config.id_args, config.ty_args_id)
        metric_names = ['TI', 'TC', 'AI', 'AC']
        for i, prf in enumerate(prf_s):

    ...
    for idx in range(0,len(model_list),2):
        ...
        for type_pred_one in type_pred:
            ...
            for i in trigger_s:
                ...
                if len(es) > 0:
                    e = es[0]
                    trigger_spans_str.append('_'.join([str(x) for x in (type_pred_one, i, e)]))
    res_trig = Counter(trigger_spans_str)
    for pred,cnt in res_trig.items():
        if cnt>len(model_list)/4:
            trigger_spans.append(pred)
    for idx in range(0,len(model_list),2):
